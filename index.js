...
async function simulate(){
    ...
    while(true){
        ...
        document.querySelector(`#cell-${x}-${y}`).classList.add('cell-current');
        // sleep for 1 second
        await new Promise(resolve => setTimeout(resolve, 300/SIMULATION_SPEED));
        // for(let i=0; i<NROWS; i++){
        //     for(let j=0; j<NCOLS; j++){
                ...
                // remove this         document.querySelector(`#cell-${x}-${y}`).style.backgroundColor = '#8b43cf';
                document.querySelector(`#cell-${x}-${y}`).classList.remove('cell-current');
                if(currValue > 0.5){
